...
<template>
  <div class="product-card relative flex h-full flex-col">
    ...
    <BaseTypography class="mb-2 text-sm md:text-base">
      <div class="flex h-full flex-col pt-4">
        <span class="font-bold">{{ product.name }}</span>
        <div class="mt-auto flex h-11 items-center justify-between">
          <MoneyValue :value="price.value" :currency="price.currency" />
          <UButton
            color="green"
            variant="ghost"
            size="lg"
